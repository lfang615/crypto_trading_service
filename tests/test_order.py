...
async def test_place_order_unauthenticated(client: AsyncClient, limit_order: json):
    response = await client.post(url="/order/place_order/", json=limit_order)  # Replace '...' with your test data
...
async def test_place_order_authenticated(client: AsyncClient, test_user_token: str, mock_get_user, limit_order: json):
    ...
    response = await client.post("/order/place_order/", json=limit_order, headers=headers)
    print(response.json())
